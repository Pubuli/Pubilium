...
contract Pubuli is ERC721URIStorage, Ownable {
    ...
    constructor(address dividendContract_,address bonusContract_) ERC721("Pubuli", "PBN") {
    ...
    returns(bool)
    {
        require(bytes(tokenURI(tokenID)).length == 0,"PuBuLi: Signed PFD URI already set!");
    ...
    function maxTokenId() public view
    returns(uint256)
    {
    ...
    returns(bool)
    {
        require(_msgSender()==_nftOperatorWallets[tokenID],"PuBuLi: Sender must be operator!");
        IERC20 token=IERC20(_dividendContract);
        require(token.allowance(_msgSender(), address(this))>=dividend,"PuBuLi: Sender must approve dividend!");
    ...
    returns(bool)
    {
        require(_msgSender()==_nftGuarantorWallets[tokenID],"PuBuLi: Sender mustbe guarantor!");
        require(ownerOf(tokenID)==_msgSender(),"PuBuLi: Sender must have this token!");
        require(_totalNftDividendBalances[tokenID]==0,"PuBuLi: token must not have dividend!");
        _burn(tokenID);
        emit Burned(tokenID);
        return true;
    }
